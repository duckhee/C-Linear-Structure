...
extern UserData **g_idxListAge = NULL;
extern UserData **g_idxListName = NULL;
...
/** 저장된 데이터의 갯수를 업데이트 하는 함수 */
unsigned int reCalcListCount(void) {
    unsigned int cnt = 0;
    UserData *pTmp = g_HeaderNode.pNext;
...
void **makeIndexByName(int *pCount) {
    /** 인자로 받은 변수에 대한 초기화 */
    *pCount = 0;
    /** list 에 값이 없을 경우 NULl 반환 */
    if (isEmpty()) {
        return NULL;
    }
    UserData **pNameIndexList = NULL;
    unsigned int length = getListCount();
    pNameIndexList = malloc(sizeof(UserData *) * length);
    memset(pNameIndexList, '\0', sizeof(UserData *) * length);
    *pCount = length;

    UserData *pTemp = g_HeaderNode.pNext;
    for (int i = 0; pTemp != &g_TailNode; ++i) {
        pNameIndexList[i] = pTemp;
        pTemp = pTemp->pNext;
    }
    for (unsigned int i = 0; i < length - 1; i++) {
        for (unsigned int j = i + 1; j < length; j++) {
            /** bubble sort */
            if (strcmp(pNameIndexList[i]->name, pNameIndexList[j]->name) > 0) {
                pTemp = pNameIndexList[i];
                pNameIndexList[i] = pNameIndexList[j];
                pNameIndexList[j] = pTemp;
            }
        }
    }
    return pNameIndexList;
}
...
void UpdateIndexAll() {
    ...
    g_idxListName = (UserData **) makeIndexByName(&cnt);
